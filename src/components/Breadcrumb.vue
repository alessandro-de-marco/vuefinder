  ...
  <div class="flex p-1.5 bg-neutral-100 dark:bg-gray-800 border-t border-b border-neutral-300 dark:border-gray-700/50 items-center select-none text-sm">
    <span :aria-label="t('Go up a directory')" data-microtip-position="bottom-right" role="tooltip">
      <svg
        ...
        @click="!isGoUpAvailable() || emit('vf-level-up')"
    ...
    <span :aria-label="t('Refresh')" data-microtip-position="bottom-right" role="tooltip" v-if="!app.loading && !isLoading">
      <svg @click="emit('vf-refresh');" class="h-6 w-6 p-1 rounded text-slate-700 hover:bg-neutral-300 dark:text-neutral-200 dark:hover:bg-gray-700 cursor-pointer" xmlns="http://www.w3.org/2000/svg" viewBox="-40 -40 580 580" fill="currentColor">
    ...
    <div v-if="!searchMode" class="group flex bg-white dark:bg-gray-700 items-center rounded p-1 ml-2 w-full" @click.self="enterSearchMode">
      ...
      <div class="flex leading-6">
        <div v-for="(item, index) in breadcrumb" :key="index">
          <slot name="breadcrumb" :item="item">
            ...
            <span
              ...
              class="px-1.5 py-1 text-slate-700 dark:text-slate-200 hover:bg-neutral-100 dark:hover:bg-gray-800 rounded cursor-pointer" :title="item.basename"
              @click="emit('vf-home', {params:{path:item.path}})"
            >
              {{ item.name }}
            </span>
          </slot>
      ...
      <svg class="animate-spin p-1 h-6 w-6 text-white ml-auto" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"  v-if="app.loading || isLoading">
...
const isGoUpAvailable = () => {
  return props.breadcrumb.length && !searchMode.value;
};
...
const handleDropZone = (e, index = null) => {
  ...
  index ??= props.breadcrumb.length - 2;
  ...
  app.emitter.emit('vf-modal-show', {
    type: 'move',
    items: {from: draggedItems, to: props.breadcrumb[index] ?? {path: (app.adapter + '://')}}
  });
};
